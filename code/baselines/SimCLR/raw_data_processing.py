...
def process_motion_sense_accelerometer_files(accelerometer_data_folder_path):
    ...
    for trial_folder in all_trials_folders:
        ...
        print(trial_folder)
        trial_folder_gyr = trial_folder.replace("B_Accelerometer_data", "C_Gyroscope_data")
        print(trial_folder_gyr)
        
        # Loop through files for every user of the trail
        for trial_user_file in sorted(glob.glob(trial_folder + "/*.csv")):
            trial_user_file_gyr = trial_user_file.replace("B_Accelerometer_data", "C_Gyroscope_data")
            ...
            if user_id_match is not None:
                ...
                # Read file gyroscope
                trial_user_file_gyr = "/home/elian.riveros/dl-13-elian/notebooks/workspaces/lstm-har/TFC-pretraining-1/code/baselines/SimCLR/" + trial_user_file_gyr
                user_trial_dataset_gyr = pd.read_csv(trial_user_file_gyr)
                user_trial_dataset_gyr.dropna(how = "any", inplace = True)

                # Extract the x, y, z channels
                values_gyr = user_trial_dataset_gyr[["x", "y", "z"]].values

                # Concatenate accelerometer and gyroscope values
                if (values.shape[0] > values_gyr.shape[0]):
                    values = values[:values_gyr.shape[0], :]
                else:
                    values_gyr = values_gyr[:values.shape[0], :]
                values = np.concatenate((values, values_gyr), axis=1)
